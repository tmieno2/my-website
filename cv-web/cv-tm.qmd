...
::: {.column width="50%"}
[Email: tmieno2@nebraska.edu](mailto: tmieno2@nebraska.edu)\
...
# mybib <- RefManageR::ReadBib(here::here("Website/cv-web/data/publications.bib"))
...
jsonlite::fromJSON("data/external_grants.json") %>%
  # jsonlite::fromJSON("cv-web/data/external_grants.json") %>%
  data.table() %>%
...
total_amount <- 
  jsonlite::fromJSON("data/external_grants.json") %>%
  data.table() %>%
  .[, amount := gsub("\\$","",amount) %>% gsub(",", "", .) %>% as.numeric()] %>%
  .[, sum(amount)]
```
